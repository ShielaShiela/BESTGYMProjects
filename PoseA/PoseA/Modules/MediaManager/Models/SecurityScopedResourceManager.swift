...
class SecurityScopedResourceManager {
    ...
    func startAccessing(_ url: URL) -> Bool {
        // If file is sandbox, no SSR is needed
        if isFileInsideAppSandbox(url) {
            log("Sandbox file detected. No SSR access required for: \(url.lastPathComponent)", level: .info)
            return true
        }

        // External file: Proceed with SSR access management
    ...
    func isFileInsideAppSandbox(_ url: URL) -> Bool {
        let sandboxPaths = [
        ...
        for sandboxPath in sandboxPaths {
            if url.path.hasPrefix(sandboxPath) {
                return true
            }
        }
        return false
