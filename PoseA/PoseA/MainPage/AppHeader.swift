...
struct AppHeader: View {
    ...
    private var analysisModeHeaderControls: some View {
        HStack {
            ...
            if cameraManager.totalFrames > 0 && !hasKeypoints && !appState.isProcessing && !ROIModel.isROIMode {
                Button(action: processActionWithTiming) {
    ...
    func processActionWithTiming() {
        ...
        if ROIModel.isROIAvailable {
            // Set ROI in the processor using roiImageSpace (not currentROI)
            if let roiRect = ROIModel.roiImageSpace {  // Remove the $ sign
                appState.poseProcessor.setROI(roiRect)
            }
            
            // Process with ROI and timing
            appState.poseProcessor.processFramesWithBasicTiming(
                from: cameraManager,
                ...
                completion: { success, error in
                    DispatchQueue.main.async {
                        ...
                        if success {
                            let stats = VitPoseProcessor.getInferenceTimeStats()
                            print("🎯 ROI Pose detection completed with timing stats:\n\(stats)")
                            appState.showKeypoints = true
                            appState.hasImportedKeypoints = false
                        } else {
                            print("❌ ROI Pose detection failed: \(error?.localizedDescription ?? "Unknown error")")
                        }
                    }
                }
            )
        } else {
            // Regular processing with timing
            appState.poseProcessor.processFramesWithBasicTiming(
                from: cameraManager,
                progress: { progress in
                    print("Processing progress: \(Int(progress * 100))%")
                ...
                completion: { success, error in
                    DispatchQueue.main.async {
                        ...
                        if success {
                            let stats = VitPoseProcessor.getInferenceTimeStats()
                            print("🎯 Pose detection completed with timing stats:\n\(stats)")
                            appState.showKeypoints = true
                            appState.hasImportedKeypoints = false
                        } else {
                            print("❌ Pose detection failed: \(error?.localizedDescription ?? "Unknown error")")
                        }
                    }
                }
            )
