...
class CameraControllerUI: NSObject, ObservableObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    private func findBestFormat(for device: AVCaptureDevice, resolution: CameraConfiguration.Resolution, frameRate: CameraConfiguration.FrameRate) -> AVCaptureDevice.Format? {
        // Sort formats by resolution quality (prefer higher resolutions)
        let sortedFormats = device.formats.sorted { format1, format2 in
            let width1 = format1.formatDescription.dimensions.width
            let width2 = format2.formatDescription.dimensions.width
            return width1 > width2
        }
        
        // Find format that matches our requirements
        return sortedFormats.first { format in
            let dimensions = format.formatDescription.dimensions
            let hasRequiredResolution = dimensions.width >= resolution.width && dimensions.height >= resolution.height
            ...
            return hasRequiredResolution && hasRequiredPixelFormat && hasDepthSupport && isNotBinned && hasRequiredFrameRate
        }
    }
    ...
    private func findBestBasicFormat(for device: AVCaptureDevice, resolution: CameraConfiguration.Resolution, frameRate: CameraConfiguration.FrameRate) -> AVCaptureDevice.Format? {
    ...
    private func setupBasicCameraInternal(frameRate: CameraConfiguration.FrameRate) {
        ...
        if let format = findBestBasicFormat(for: device, resolution: currentResolution, frameRate: frameRate) {
            do {
                try device.lockForConfiguration()
                device.activeFormat = format
...
//            print("Frame \(deliveredFrameCount): Interval = \(String(format: "%.3f", interval))s, FPS = \(String(format: "%.1f", fps))")
//        }
//        lastFrameTimestamp = now
        ...
        let cameraReferenceDimensions = CGSize(width: CGFloat(CVPixelBufferGetWidth(pixelBuffer)),
