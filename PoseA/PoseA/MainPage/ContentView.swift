...
//                                loadData(from: url)
                                loadDataFromFilePicker(from: url)  // ✅ Uses security-scoped access
                            }
                        }
                    }
                    ...
                    .sheet(isPresented: $appState.isPhotoLibraryPresented) {
                        PhotoLibraryVideoPicker(isPresented: $appState.isPhotoLibraryPresented) { url in
                            if let url = url {
                                // Use loadVideo Function
//                                loadData(from: url)
                                loadDataFromPhotoLibrary(from: url)  // ✅ No security-scoped access
    ...
    // Alternative approach: Create separate methods for different sources
    private func loadDataFromFilePicker(from url: URL) {
        // This method handles file picker URLs that need security-scoped access
        ...
        // Start accessing the selected resource (needed for file picker)
        ...
        // Continue with common loading logic
        loadDataCommon(from: url, isDirectory: &isDirectory, fileManager: fileManager)
    }

    private func loadDataFromPhotoLibrary(from url: URL) {
        // This method handles photo library URLs that don't need security-scoped access
        appState.isProcessing = true
        appState.processingStatus = "Loading video from library..."
        ...
        print("📱 Loading from photo library - no security-scoped access needed")
        
        // Continue with common loading logic
        loadDataCommon(from: url, isDirectory: &isDirectory, fileManager: fileManager)
    }

    private func loadDataCommon(from url: URL, isDirectory: inout ObjCBool, fileManager: FileManager) {
        ...
        if isDirectory.boolValue {
            // It's a folder - load as LiDAR data
            print("📁 Loading folder as LiDAR data: \(url.lastPathComponent)")
            appState.isVideoSource = false // LiDAR data
            ...
            case "json":
                print("📄 Loading keypoint file: \(url.lastPathComponent)")
                loadKeypointFile(url)
                
            case "mp4", "mov", "m4v":
                print("🎬 Loading video file: \(url.lastPathComponent)")
                appState.isVideoSource = true // Video data
                loadVideoFromFile(url)
...
//            switchToAnalysisMode()
    ...
    func updateDisplayWithKeypoints(for frameIndex: Int) {
        print("🔍 Updating display with keypoints for frame \(frameIndex)...")
        
        // CRITICAL: Check if we actually have an image before proceeding
        guard let currentImage = cameraManager.currentFrameImage else {
            print("❌ No image found for frame \(frameIndex) - currentFrameImage is nil")
            // DON'T trigger another frame load here - this causes the infinite loop!
            return
        }
        
        print("✅ Image found for frame \(frameIndex) - Size: \(currentImage.size)")
        
        // Continue with your keypoint display logic...
        // Make sure this method doesn't call setFrame again!
    }
    ...
    private func loadVideoFromFile(_ url: URL) {
        print("🎬 Starting video load from: \(url.path)")
        
        // Use the unified camera manager method
        cameraManager.loadVideoFile(url) { success, totalFrames, error in
            DispatchQueue.main.async {
                // No [weak self] needed for structs - just use self directly
                self.appState.isProcessing = false
                ...
                if success {
                    ...
                    print("✅ Video loaded successfully: \(totalFrames) frames")
                    
                    // Check for associated keypoint files after a short delay
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                        self.checkForAssociatedKeypointFile(videoURL: url)
                    }
                } else if let error = error {
                    print("❌ Video loading failed: \(error.localizedDescription)")
                    self.appState.errorMessage = "Failed to load video: \(error.localizedDescription)"
                }
            }
        }
    }
